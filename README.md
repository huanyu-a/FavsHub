...
- [FavsHub最新项目包 (ZIP)](https://github.com/huanyu-a/FavsHub/releases/)
...
## 📖最近更新  **[查看详细更新记录 →](CHANGELOG.md)**
## [v2.1.1] - 2026-05-08

### ✨ 新增功能

- **功能暂无法使用**：百度网盘应用正在提交审核中，预计1-7天出结果

#### 百度网盘云端备份
- **一键授权连接**：点击"连接百度网盘"即可完成 OAuth 授权，无需输入任何凭据
- **云端备份目录**：`/apps/FavsHub/`
...
| `tabs` | 创建和管理标签页 |
| `identity` | 百度网盘 OAuth 授权 |

**🔒 安全承诺**: 所有数据默认仅存储在本地或 Chrome 同步存储中。百度网盘云端备份为可选功能，需用户主动授权后才会上传数据到用户自己的百度网盘。
...
### Q: PromptPro 的数据如何备份？
A: 支持两种方式：① 本地文件夹备份（设置中授权本地目录）；② 百度网盘云端备份（设置中点击"连接百度网盘"授权后自动/手动备份）。也可点击左侧边栏"📤 导出数据"按钮手动导出 JSON 文件。
