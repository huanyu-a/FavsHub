...
- **本地文件夹自动备份**：通过浏览器 File System Access API 授权本地文件夹，将 PromptPro 数据自动备份到指定目录
  ...
  - 目录句柄持久化存储，刷新后无需重新授权
- **统一备份数据格式**：导出和自动备份使用统一的 JSON 格式，包含完整版本历史
- **PromptPro 页面导出增强**：导出按钮改为写入备份文件夹，支持数据哈希比对
- **PromptPro 导入兼容**：导入支持新旧两种备份格式，导入后自动刷新页面

### 🏗️ 架构优化

- **数据库统一**：备份配置从独立的 FavsHubBackupDB 合并到 PromptProDB 的 `backup_config` 表
- **存储字段整合**：`lastBackupTime`、`lastBackupHash` 合并到每条备份记录中，移除冗余顶层 key

### 🌐 国际化

- 新增备份相关的 12 个 i18n 字符串（中/英）
